...
resolve_artifact iso_path '*.install.iso' 'install ISO'
...
if [[ -f "$inspect_dir/rpm-query.rc" ]]; then
    ...
    if [[ "$rpm_query_rc" == 0 ]]; then
        ...
        fail "rpm query inspection failed with rc=$rpm_query_rc"
    fi
fi
...
if [[ -n "$qcow2_path" ]] && [[ -n "$iso_path" ]] && command -v file >/dev/null 2>&1; then
    file "$iso_path" "$qcow2_path" >"$inspect_dir/file-output.txt"
